"""Teach Ask Nevo to look things up rather than answer from what it was handed.
...
# Shape guidance from 0035, unchanged: the server normalises whatever comes
# back, so the model is asked to match the shape to the answer.
SHAPE = (
...
# Without this the model answers from the seed context and never calls a tool,
# which is the behaviour that made it look incurious.
TOOLS = (
...
STUDENT = (
    ...
    "functional learning language only." + SHAPE + TOOLS
)
TEACHER = (
    ...
    "Use functional learning language only." + SHAPE + TOOLS
)
...
INSERT_VERSION = """
    ...
    SELECT service, name, 3, $tpl${system}$tpl$, user_template,
...
def downgrade() -> None:
    for name in ("ask_nevo.student", "ask_nevo.teacher"):
        op.execute(
            f"DELETE FROM ai_prompt_templates WHERE name = '{name}' AND version = 3"
        )
        op.execute(
            f"UPDATE ai_prompt_templates SET active = true "
            f"WHERE name = '{name}' AND version = 2"
        )
