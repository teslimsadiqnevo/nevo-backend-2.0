...
Nevo needs one controlled boundary for model calls. Direct provider calls from
...
- An in-process priority scheduler orders remaining model-backed work while
  enforcing configured concurrency and RPM.
...
- Claude credentials are sent in the `x-api-key` header, not the URL.
- Claude prompt caching is enabled by default and cache write/read token usage
  is audited separately from fresh input tokens.
- Mastery, affective inference, spaced repetition, UDL accommodation, break
  logic, and adaptation stay local and do not call this gateway.

## Consequences

Feature code depends on a provider-neutral service and cannot bypass policy by
using a provider directly. Provider limits still apply across replicas; the
current process-local scheduler protects each replica and can be replaced by a
distributed implementation without changing callers.
