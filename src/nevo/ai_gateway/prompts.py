...
class PromptRenderer:
    def render(
        ...
        clean_variables = {key: str(value).strip() for key, value in variables.items()}
        try:
            system_instruction = template.system_template.format_map(clean_variables)
    ...
    def referenced_variables(template_text: str) -> frozenset[str]:
        return frozenset(
            field_name for _, field_name, _, _ in Formatter().parse(template_text) if field_name
        )
